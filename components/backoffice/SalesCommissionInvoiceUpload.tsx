...
export default function SalesCommissionInvoiceUpload({ period, amount }: { period: string; amount: string }) {
  ...
  return <div className="mt-2 rounded-xl border border-[#E1CEAD] bg-[#FFF8EC] p-3">
    ...
    <div className="mt-2 flex items-center justify-between gap-3">{message ? <p className="text-[10px] font-semibold text-[#7A592F]">{message}</p> : <p className="text-[9px] text-[#8A7863]">O pagamento só é libertado após validação da fatura e do IBAN.</p>}<button type="button" onClick={save} disabled={saving || uploading || !invoiceUrl || !invoiceNumber.trim()} className="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-lg bg-[#17130F] px-3 text-[10px] font-bold text-white disabled:opacity-40">{saving && <Loader2 size={11} className="animate-spin" />} Enviar</button></div>
  </div>;
}
