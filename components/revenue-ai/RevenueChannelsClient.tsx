  ...
  return <div className="space-y-5">
    <section className="rounded-[26px] border border-[#E1D0B8] bg-white p-5">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <SimpleChannel icon={<MessageCircleMore size={20} />} title="WhatsApp AI MesaLink" status={status.whatsappReady ? "Ativo" : request?.channels.includes("WHATSAPP") ? requestStatusLabel : "Extra opcional"} text="A conversa usa um número atribuído ao restaurante e fica guardada na caixa Revenue AI." price={`≈ 0,03€ por mensagem · saldo ${whatsappBalance}`} active={status.whatsappReady} pending={Boolean(request?.channels.includes("WHATSAPP") && request.status !== "COMPLETED" && !status.whatsappReady)} />
    ...
    {(!status.whatsappReady || !status.voiceReady) && <section className="rounded-[26px] border border-[#D7B267] bg-[#FFF7E8] p-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#9B6F3B]">Ativar extras</p><h2 className="mt-1 text-2xl font-semibold tracking-[-0.045em]">Pedir WhatsApp ou chamadas</h2><p className="mt-1 text-xs leading-5 text-[#6B6258]">Indica o WhatsApp atual de {restaurantName}. A MesaLink atribui e configura o número usado pelos clientes.</p></div>{request && <span className="inline-flex shrink-0 items-center gap-2 rounded-full bg-[#F1E0B9] px-4 py-2 text-xs font-black text-[#715021]"><CheckCircle2 size={15} /> {requestStatusLabel}</span>}</div>
      <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_1fr_1.1fr_auto]">
        ...
        <label className="rounded-[22px] border border-[#DFC9A5] bg-white p-4"><span className="text-[10px] font-black uppercase tracking-[0.13em] text-[#79664E]">WhatsApp atual do restaurante</span><input value={contactPhone} onChange={(event) => setContactPhone(event.target.value)} placeholder="+351 912 345 678" inputMode="tel" className="mt-2 h-11 w-full bg-transparent text-sm font-bold outline-none" /><span className="text-[10px] leading-4 text-[#8A7863]">Recebe alertas, chamadas encaminhadas e pedidos de passagem a humano.</span></label>
