...
export default function RecoveryOfferButton({ restaurantId, label = "Configurar recuperação" }: { restaurantId: string; label?: string }) {
  const [open, setOpen] = useState(false);
  const [sending, setSending] = useState(false);
  const [message, setMessage] = useState("");
  const [success, setSuccess] = useState(false);
  const [benefitType, setBenefitType] = useState("PERCENT");
  const [template, setTemplate] = useState<MarketingCardTheme>("FOREST");
  ...
  async function send(event: FormEvent<HTMLFormElement>) {
    ...
    setSending(false);
    if (!response.ok || !data?.success) return setMessage(data?.error || "Não foi possível executar a recuperação.");
    setSuccess(true);
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[130] flex items-end justify-center bg-[#17120D]/55 p-0 backdrop-blur-sm sm:items-center sm:p-5" onMouseDown={(event) => { if (event.target === event.currentTarget && !sending) setOpen(false); }}>
      <form onSubmit={send} className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-t-[30px] border border-[#E1D0B8] bg-[#FFF9F0] shadow-2xl sm:rounded-[30px]">
        <header className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-[#E1D0B8] bg-white/95 p-5 backdrop-blur"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#9B6F3B]">Recuperação de clientes</p><h3 className="mt-1 text-2xl font-semibold tracking-[-0.04em]">Escolhe o incentivo antes de enviar</h3><p className="mt-1 text-xs text-[#75695D]">Cada cliente inativo recebe o seu próprio cartão digital.</p></div><button type="button" onClick={() => setOpen(false)} disabled={sending} className="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-[#E1D0B8] bg-[#FFF9F0]"><X size={15} /></button></header>
        <div className="space-y-5 p-5">
          <div><p className="text-[10px] font-black uppercase tracking-[0.12em] text-[#806D56]">Tipo de incentivo</p><div className="mt-2 grid grid-cols-3 gap-2">{[{ value: "PERCENT", label: "Desconto %" }, { value: "FIXED", label: "Desconto €" }, { value: "GIFT", label: "Oferta" }].map((option) => <button key={option.value} type="button" onClick={() => setBenefitType(option.value)} className={`rounded-2xl border px-3 py-3 text-xs font-bold transition ${benefitType === option.value ? "border-[#B9853E] bg-[#FFF0D3]" : "border-[#E1D0B8] bg-white"}`}>{option.label}</button>)}</div></div>
          <div className="grid gap-3 sm:grid-cols-3"><label className="text-xs font-bold text-[#75695D]">{benefitType === "PERCENT" ? "Percentagem" : benefitType === "FIXED" ? "Valor em euros" : "Oferta selecionada"}<input name="value" type="number" min="1" max={benefitType === "PERCENT" ? 50 : 1000} step={benefitType === "FIXED" ? "0.01" : "1"} defaultValue="10" disabled={benefitType === "GIFT"} className="input-premium mt-2 disabled:bg-[#EEE6DB]" /></label><label className="text-xs font-bold text-[#75695D]">Validade<select name="validDays" defaultValue="30" className="input-premium mt-2"><option value="14">14 dias</option><option value="30">30 dias</option><option value="60">60 dias</option><option value="90">90 dias</option></select></label><label className="text-xs font-bold text-[#75695D]">Consumo mínimo €<input name="minSpend" type="number" min="0" max="10000" step="0.01" placeholder="Opcional" className="input-premium mt-2" /></label></div>
          <label className="block text-xs font-bold text-[#75695D]">Título do cartão<input name="title" required maxLength={100} defaultValue="Um convite especial para voltar" className="input-premium mt-2" /></label>
          <label className="block text-xs font-bold text-[#75695D]">Mensagem do cartão<textarea name="description" required maxLength={280} rows={3} defaultValue="Já passou algum tempo desde a sua última visita. Gostávamos muito de o voltar a receber." className="input-premium mt-2 resize-none py-3" /></label>
