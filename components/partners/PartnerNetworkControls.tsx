...
export function ReferralBookingSettingsForm({
  ...
  async function saveDate(enabled: boolean) {
    ...
    if (enabled && (!Number.isInteger(capacity) || capacity < 1 || capacity > totalCapacity)) return setMessage(`Indica um limite entre 1 e ${totalCapacity} lugares.`);
    ...
    <div className="space-y-5">
      ...
      <form onSubmit={submit} className="space-y-5">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <label className="block min-w-0 rounded-[18px] border border-[#E5D7C3] bg-[#FBF7F1] p-4"><span className="text-[9px] font-black uppercase tracking-[0.16em] text-[#9B6F3B]">Comissão padrão</span><div className="mt-2 grid min-w-0 grid-cols-[minmax(0,1fr)_82px] gap-2"><select name="commissionType" defaultValue={initialCommissionType} className="h-11 min-w-0 w-full rounded-xl border border-[#DDCDB5] bg-white px-3 text-xs font-bold outline-none"><option value="PER_PERSON">Por pessoa</option><option value="TOTAL">Total da reserva</option></select><div className="relative min-w-0"><input name="commissionAmount" type="number" min="0.5" max="1000" step="0.01" defaultValue={initialCommissionAmount} className="h-11 min-w-0 w-full rounded-xl border border-[#DDCDB5] bg-white pl-3 pr-7 text-sm font-bold outline-none" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-[#8B765E]">€</span></div></div><p className="mt-2 text-[9px] leading-4 text-[#7A6C5D]">O valor que todos os parceiros veem, salvo acordo próprio. Acresce IVA.</p></label>
          <div className="block rounded-[18px] border border-[#E5D7C3] bg-[#F3EDE4] p-4"><span className="text-[9px] font-black uppercase tracking-[0.16em] text-[#9B6F3B]">Capacidade total</span><div className="mt-2 flex h-11 items-center rounded-xl border border-[#DDCDB5] bg-[#E9E0D3] px-3"><UsersRound size={16} className="mr-3 text-[#A47A43]" /><strong className="text-sm">{totalCapacity}</strong><span className="ml-auto text-[10px] text-[#8B765E]">lugares</span></div><p className="mt-2 text-[9px] leading-4 text-[#7A6C5D]">Definida nas mesas ou nas definições. Não é editável aqui.</p></div>
          <label className="block rounded-[18px] border border-[#E5D7C3] bg-[#FBF7F1] p-4"><span className="text-[9px] font-black uppercase tracking-[0.16em] text-[#9B6F3B]">Limite para Partners</span><div className="mt-2 flex h-11 items-center rounded-xl border border-[#DDCDB5] bg-white"><UsersRound size={16} className="ml-3 text-[#A47A43]" /><input name="defaultDailyCapacity" type="number" min="1" max={totalCapacity} step="1" defaultValue={initialDefaultDailyCapacity} className="h-full min-w-0 flex-1 bg-transparent px-3 text-sm font-bold outline-none" /><span className="pr-3 text-[10px] text-[#8B765E]">lugares</span></div><p className="mt-2 text-[9px] leading-4 text-[#7A6C5D]">Máximo de lugares Partner ocupados no mesmo horário.</p></label>
      ...
      <div className="border-t border-[#E9DECF] pt-5">
        ...
        <div className="mt-3 grid gap-2 rounded-[18px] bg-[#F5EEE4] p-3 sm:grid-cols-[170px_130px_auto_auto] sm:items-center">
          ...
          <input value={exceptionCapacity} onChange={(event) => setExceptionCapacity(event.target.value)} type="number" min="1" max={totalCapacity} placeholder="Limite Partner" aria-label="Limite de lugares Partner" className="h-10 rounded-xl border border-[#DDCDB5] bg-white px-3 text-xs font-semibold outline-none" />
          <button type="button" onClick={() => saveDate(true)} disabled={capacityLoading} className="inline-flex h-10 items-center justify-center gap-2 rounded-full border border-[#CBB795] bg-white px-4 text-[10px] font-bold disabled:opacity-50"><Check size={13} /> Definir limite</button>
...
export function PartnerProfileSettingsForm({
  ...
  googleMapsAssociated: boolean;
}) {
  const [associationOpen, setAssociationOpen] = useState(false);
  const [placeQuery, setPlaceQuery] = useState(restaurantName);
  const [matches, setMatches] = useState<Array<{ placeId: string; name: string; address: string; rating: number | null; reviewCount: number | null; mapUrl: string }>>([]);
  const [associationLoading, setAssociationLoading] = useState(false);
  const [associationSaving, setAssociationSaving] = useState("");
  ...
  async function searchGoogleMaps() {
    ...
    try {
      const params = new URLSearchParams({ q: placeQuery || restaurantName, location: address || "Portugal" });
      const response = await fetch(`/api/restaurants/${restaurantId}/google-places/search?${params}`);
      const data = await response.json().catch(() => null);
      if (!response.ok) return setAssociationMessage(data?.error || "Não foi possível pesquisar agora.");
      setMatches(Array.isArray(data?.restaurants) ? data.restaurants : []);
      if (!data?.restaurants?.length) setAssociationMessage("Não encontrámos resultados. Experimenta o nome completo do restaurante.");
    } catch {
      setAssociationMessage("Não foi possível pesquisar agora.");
    } finally {
      setAssociationLoading(false);
    }
  }

  async function associateGoogleMaps(placeId: string) {
    setAssociationSaving(placeId);
    setAssociationMessage("");
    try {
      const response = await fetch(`/api/restaurants/${restaurantId}/google-places/associate`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ placeId }),
      });
      const data = await response.json().catch(() => null);
      if (!response.ok) return setAssociationMessage(data?.error || "Não foi possível associar agora.");
      window.location.assign(`/restaurants/${restaurantId}/partner-network?google=maps-linked`);
    } catch {
      setAssociationMessage("Não foi possível associar agora.");
    } finally {
  ...
  return (
    <div className="overflow-hidden rounded-[24px] bg-[#17120D] text-white shadow-[0_22px_55px_rgba(35,24,14,0.18)]">
      <div className="relative h-48 overflow-hidden bg-[#3B2A1C] sm:h-56">{heroImage ? <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `linear-gradient(180deg,rgba(23,18,13,.05),rgba(23,18,13,.72)),url(${heroImage})` }} /> : <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,#785B35,transparent_40%),linear-gradient(135deg,#2A1D13,#17120D)]" />}<div className="absolute left-4 top-4 flex items-center gap-2 rounded-full border border-white/20 bg-black/30 px-3 py-1.5 text-[8px] font-black uppercase tracking-[0.14em] backdrop-blur"><span className={`h-2 w-2 rounded-full ${googleMapsAssociated ? "bg-[#93D297]" : "bg-[#E7BC67]"}`} />{googleMapsAssociated ? "Ligado ao Google Maps" : "Associar ao Google Maps"}</div><div className="absolute inset-x-5 bottom-5"><p className="text-[9px] font-black uppercase tracking-[0.18em] text-[#E6C57E]">Como aparece aos parceiros</p><h3 className="mt-2 text-2xl font-semibold leading-tight tracking-[-0.04em]">{restaurantName}</h3></div></div>
      <div className="p-5"><div className="flex flex-wrap items-center gap-x-3 gap-y-2 text-[10px] font-bold"><span className="rounded-full bg-white/10 px-3 py-1.5">{cuisine || "Restaurante"}</span>{googleRating != null && <span className="text-[#F1CF80]">★ {googleRating.toFixed(1)} <span className="font-normal text-white/45">({googleReviewCount || 0} avaliações)</span></span>}{googlePriceLevel && <span className="tracking-[0.12em] text-[#A7D0A9]">{"€".repeat(googlePriceLevel)}</span>}</div>{address && <p className="mt-4 flex items-start gap-2 text-[10px] leading-5 text-white/60"><MapPin size={14} className="mt-0.5 shrink-0 text-[#D6AF64]" />{address}</p>}{description && <p className="mt-3 line-clamp-3 text-[11px] leading-5 text-white/65">{description}</p>}<div className="mt-5 grid grid-cols-2 gap-2"><button type="button" onClick={() => { if (associationOpen) setAssociationOpen(false); else void searchGoogleMaps(); }} className="inline-flex h-10 items-center justify-center gap-2 rounded-full bg-[#D6AF64] px-4 text-[9px] font-black text-[#17120D]">{googleMapsAssociated ? <RefreshCw size={13} /> : <ShieldCheck size={13} />}{googleMapsAssociated ? "Alterar ligação" : "Associar Google"}</button>{googleMapsUrl ? <a href={googleMapsUrl} target="_blank" rel="noreferrer" className="inline-flex h-10 items-center justify-center gap-2 rounded-full border border-white/20 px-4 text-[9px] font-bold text-white"><ExternalLink size={12} /> Abrir Maps</a> : <span className="inline-flex h-10 items-center justify-center rounded-full border border-white/10 px-4 text-center text-[8px] text-white/35">Maps ainda não associado</span>}</div>
        {associationOpen && <div className="mt-4 rounded-2xl border border-white/15 bg-white/5 p-3"><div className="flex gap-2"><input value={placeQuery} onChange={(event) => setPlaceQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void searchGoogleMaps(); } }} placeholder="Nome do restaurante" className="h-10 min-w-0 flex-1 rounded-full border border-white/15 bg-white/10 px-4 text-xs text-white outline-none placeholder:text-white/35" /><button type="button" onClick={() => void searchGoogleMaps()} disabled={associationLoading} className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-white text-[#17120D] disabled:opacity-50">{associationLoading ? <Loader2 size={14} className="animate-spin" /> : <Search size={14} />}</button></div>{matches.length > 0 && <div className="mt-3 space-y-2">{matches.map((place) => <div key={place.placeId} className="rounded-xl border border-white/10 bg-black/15 p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[11px] font-bold">{place.name}</p><p className="mt-1 text-[9px] leading-4 text-white/50">{place.address}</p>{place.rating != null && <p className="mt-1 text-[9px] text-[#F1CF80]">★ {place.rating.toFixed(1)}{place.reviewCount ? ` (${place.reviewCount})` : ""}</p>}</div><button type="button" onClick={() => void associateGoogleMaps(place.placeId)} disabled={Boolean(associationSaving)} className="h-8 shrink-0 rounded-full bg-[#D6AF64] px-3 text-[8px] font-black text-[#17120D] disabled:opacity-50">{associationSaving === place.placeId ? "A associar…" : "Associar"}</button></div></div>)}</div>}{associationMessage && <p className="mt-3 text-[9px] font-semibold text-[#F1CF80]">{associationMessage}</p>}</div>}
        <p className="mt-4 flex items-start gap-2 border-t border-white/10 pt-4 text-[8px] leading-4 text-white/40"><ShieldCheck size={12} className="mt-0.5 shrink-0" />Associa o perfil correto para usar a localização, avaliações e ligação ao Maps.</p></div>
    </div>
  );
}
