...
import { CalendarDays, CheckCircle2, Clock3, RotateCcw, ShieldCheck, UtensilsCrossed, UsersRound, XCircle } from "lucide-react";
...
export default async function ManageReservationPage({ params, searchParams }: { params: Promise<{ reservationId: string }>; searchParams: Promise<{ token?: string; intent?: string; result?: string; error?: string }> }) {
  ...
  const reservation = await prisma.reservation.findUnique({
    where: { id: reservationId },
    include: { restaurant: { select: { name: true, slug: true, address: true } }, experience: true, experienceAddOns: true, payment: true },
  });
  ...
  const cancellable = ACTIVE_STATUSES.includes(reservation.status) && reservation.date > new Date();
  const manageable = cancellable && !reservation.experienceId && reservation.payment?.status !== "PAID";
  ...
  return <main className="min-h-screen bg-[#F5EFE6] px-4 py-8 text-[#17120D] sm:px-6 lg:py-14">
    <div className="mx-auto max-w-5xl">
      ...
      <section className="mt-6 overflow-hidden rounded-[38px] border border-[#DCC9AA] bg-white shadow-[0_28px_90px_rgba(73,49,25,0.1)]">
        <div className="bg-[#17120D] p-7 text-white sm:p-9"><p className="text-[10px] font-black uppercase tracking-[0.28em] text-[#D7B267]">{reservation.restaurant.name}</p><h1 className="mt-3 text-4xl font-semibold tracking-[-0.06em] sm:text-5xl">{cancelled ? "Reserva cancelada" : cancellable ? "A sua reserva, sob controlo." : "Esta reserva já terminou."}</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-white/60">{cancelled ? "O lugar foi libertado. Se os planos mudarem, pode escolher já uma nova data." : cancellable ? "Consulte todos os dados e, se não puder comparecer, cancele a reserva aqui." : "Para qualquer questão adicional, contacte diretamente o restaurante."}</p></div>
      ...
      {reservation.experience && <section className="mt-6 flex flex-col gap-5 rounded-[30px] border border-[#DCC9AA] bg-white p-6 sm:flex-row sm:items-center sm:justify-between sm:p-7"><div className="flex min-w-0 gap-4"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#F3E7D5] text-[#9B6F3B]"><UtensilsCrossed size={19}/></span><div><p className="text-[9px] font-black uppercase tracking-[0.22em] text-[#9B6F3B]">Menu incluído na reserva</p><h2 className="mt-1 text-2xl font-semibold tracking-[-0.04em]">{reservation.experience.title}</h2><p className="mt-1 text-sm text-[#6B6258]">{formatMoney(Number(reservation.experience.pricePerPerson))} por pessoa{reservation.experienceAddOns.length ? ` · Extras: ${reservation.experienceAddOns.map((item) => `${item.nameSnapshot} × ${item.quantity}`).join(", ")}` : ""}</p></div></div><span className="shrink-0 rounded-full bg-[#EFF9EF] px-4 py-2 text-xs font-bold text-[#3F6A4D]">Selecionado</span></section>}

      {cancelled || !cancellable ? <section className="mt-6 rounded-[34px] border border-[#DCC9AA] bg-[#FFF9F0] p-7 text-center sm:p-9"><RotateCcw className="mx-auto text-[#9B6F3B]" /><h2 className="mt-4 text-3xl font-semibold tracking-[-0.05em]">Quer marcar noutra data?</h2><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-[#6B6258]">Consulte novamente a disponibilidade do {reservation.restaurant.name} e faça uma nova reserva em poucos passos.</p><a href={rebookUrl} className="mt-5 inline-flex h-12 items-center justify-center rounded-full bg-[#17120D] px-6 text-sm font-bold text-white">Marcar noutra data</a></section> : <div className={`mt-6 grid gap-6 ${manageable ? "lg:grid-cols-[1.15fr_0.85fr]" : "lg:grid-cols-2"}`}>
        {manageable ? <section className="rounded-[34px] border border-[#DCC9AA] bg-white p-6 sm:p-8"><p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#9B6F3B]">Alterar reserva</p><h2 className="mt-2 text-3xl font-semibold tracking-[-0.05em]">Novos planos? Sem problema.</h2><p className="mt-2 text-sm leading-6 text-[#6B6258]">Escolha a nova data, hora e número de pessoas. Se for necessária confirmação do restaurante, avisamos imediatamente.</p><form action={updateReservation} className="mt-6 grid gap-4 sm:grid-cols-2"><input type="hidden" name="reservationId" value={reservation.id} /><input type="hidden" name="token" value={token} /><Field label="Data"><input name="date" type="date" min={lisbonInputParts(new Date()).date} defaultValue={local.date} required className="input-premium" /></Field><Field label="Hora"><input name="time" type="time" defaultValue={local.time} required className="input-premium" /></Field><Field label="Número de pessoas" wide><input name="guests" type="number" min="1" max="500" defaultValue={reservation.guests} required className="input-premium" /></Field><button className="sm:col-span-2 inline-flex h-12 items-center justify-center gap-2 rounded-full bg-[#17120D] px-6 text-sm font-bold text-white"><CheckCircle2 size={16} /> Guardar alteração</button></form></section> : <section className="rounded-[34px] border border-[#DCC9AA] bg-white p-6 sm:p-8"><ShieldCheck className="text-[#9B6F3B]"/><p className="mt-5 text-[10px] font-black uppercase tracking-[0.24em] text-[#9B6F3B]">Dados protegidos</p><h2 className="mt-2 text-3xl font-semibold tracking-[-0.05em]">{reservation.experience ? "O menu está reservado." : "Pagamento confirmado."}</h2><p className="mt-3 text-sm leading-6 text-[#6B6258]">Para garantir que o menu e eventuais pagamentos continuam corretos, não alteramos esta reserva diretamente. Pode cancelá-la abaixo e fazer uma nova reserva com os dados pretendidos.</p></section>}
...
function formatMoney(value: number) { return new Intl.NumberFormat("pt-PT", { style: "currency", currency: "EUR" }).format(value); }
