  ...
  return (
    <main className="min-h-screen bg-[#F5EFE6] text-[#17120D]">
      ...
      <div className="mx-auto max-w-7xl px-4 pb-16 pt-7 sm:px-6">
        <nav className="mb-7 inline-flex rounded-full border border-[#D9C7AA] bg-white p-1"><Link href="/partners/app" className="rounded-full bg-[#17120D] px-5 py-2.5 text-xs font-bold text-white">Grupos e reservas</Link><Link href="/partners/app/benefits" className="rounded-full px-5 py-2.5 text-xs font-bold text-[#6B6258]">Cartões e benefícios</Link></nav>
        {!partner.stripeOnboardingComplete && (
          <section className="mb-6 flex flex-col gap-4 rounded-[28px] border border-[#2C2117] bg-[#17120D] p-5 text-white sm:flex-row sm:items-center sm:justify-between sm:p-6">
            <div>
              ...
              <p className="mt-1 text-xs leading-5 text-white/50">A verificação e os pagamentos são tratados pelo Stripe Connect. O MesaLink não mostra os teus dados bancários aos restaurantes.</p>
            ...
            <form action="/api/partners/connect" method="POST">
              <button className="h-12 w-full whitespace-nowrap rounded-full bg-[#D7B267] px-6 text-sm font-black text-[#17120D] sm:w-auto">Ativar pagamentos</button>
            </form>
          </section>
        )}
        ...
        <section className="mt-7 grid grid-cols-2 gap-3 lg:grid-cols-4">
          <Kpi icon={<Euro size={18} />} label="Comissões recebidas" value={formatMoney(paidRevenue)} />
          ...
          <Kpi icon={<ShieldCheck size={18} />} label="Acordos ativos" value={String(partner.agreements.length)} />
        </section>
