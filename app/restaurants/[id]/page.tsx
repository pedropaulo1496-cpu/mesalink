...
function MarketingInterventions({ t, restaurantId, growthAccess, inactiveCustomers, birthdayCustomers, poorReviewCustomers }: { t: TFunc; restaurantId: string; growthAccess: boolean; inactiveCustomers: number; birthdayCustomers: number; poorReviewCustomers: number }) {
  ...
  return <section className="mt-4 overflow-hidden rounded-[24px] border border-[#DFC89F] bg-white shadow-[0_14px_40px_rgba(80,55,30,0.04)]"><div className="flex flex-col gap-3 bg-[#FFF8EA] px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#9B6F3B]">{t("summary.marketing.interventionsEyebrow")}</p><p className="mt-1 text-sm font-semibold">{t("summary.marketing.interventionsTitle")}</p></div><span className="w-fit rounded-full bg-[#17120D] px-3 py-1.5 text-[9px] font-black uppercase tracking-[0.1em] text-white">{t("summary.marketing.opportunities", { count: opportunities })}</span></div><div className="divide-y divide-[#EEE3D3]">{interventions.length ? interventions.map((intervention) => <div key={intervention.key} className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-3"><span className="grid h-8 min-w-8 place-items-center rounded-full bg-[#F1E6D5] text-xs font-black text-[#825D2D]">{intervention.count}</span><p className="text-sm font-semibold text-[#3F372E]">{intervention.label}</p></div><div className="shrink-0">{intervention.action}</div></div>) : <div className="flex items-center gap-3 px-4 py-4"><span className="grid h-8 min-w-8 place-items-center rounded-full bg-[#ECF7EC] text-sm font-black text-[#3F6A4D]">✓</span><p className="text-sm font-semibold text-[#3F6A4D]">{t("summary.marketing.allClear")}</p></div>}</div></section>;
}
  ...
  return (
    <div className="rounded-[28px] border border-[#E1D0B8] bg-white p-4 shadow-[0_18px_55px_rgba(80,55,30,0.05)] sm:p-5">
      <div className="flex items-end justify-between gap-3">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.22em] text-[#9B6F3B]">{t("summary.upcoming.eyebrow")}</p>
          <h2 className="mt-1 text-2xl font-semibold tracking-[-0.05em]">{t("summary.upcoming.title")}</h2>
        </div>
        <Link href={`/restaurants/${restaurantId}/reservations/upcoming`} className="text-xs font-bold text-[#9B6F3B]">{t("summary.upcoming.all")} →</Link>
      </div>
      <div className="mt-4 overflow-hidden rounded-[20px] border border-[#E8DCCB]">
        {reservations.length ? reservations.map((reservation) => (
          <details key={reservation.id} className="group border-b border-[#E8DCCB] bg-[#FFFDFC] last:border-b-0">
            <summary className="grid cursor-pointer list-none grid-cols-[54px_minmax(0,1fr)_auto] items-center gap-3 px-3 py-3 transition hover:bg-white">
              <div className="rounded-xl bg-[#F1E6D5] py-2 text-center">
                <p className="text-[9px] font-black uppercase text-[#8A6D49]">{new Intl.DateTimeFormat(intlLocale, { day: "2-digit", month: "short" }).format(reservation.date)}</p>
                <p className="mt-0.5 text-xs font-black">{new Intl.DateTimeFormat(intlLocale, { hour: "2-digit", minute: "2-digit" }).format(reservation.date)}</p>
              </div>
              <div className="min-w-0">
                <div className="flex min-w-0 flex-wrap items-center gap-1.5">
                  <p className="truncate text-sm font-semibold">{reservation.customerName}</p>
                  {reservation.offer && <span className="shrink-0 rounded-full bg-[#FFF0D3] px-2 py-0.5 text-[8px] font-black uppercase text-[#7A542A]">{t("summary.upcoming.offerApplied")}</span>}
                </div>
                <p className="mt-0.5 text-[10px] text-[#776B5F]">{t("summary.upcoming.guests", { count: reservation.guests })}{reservation.status === "PENDING" ? ` · ${t("summary.upcoming.pending")}` : ""}</p>
                {reservation.menuTitle && <ReservationMenuLabel title={reservation.menuTitle} className="mt-1.5" />}
              </div>
              <span className="flex items-center gap-2 text-xs font-black text-[#9B6F3B]">{reservation.guests}p <span className="transition group-open:rotate-180">⌄</span></span>
            </summary>
            <div className="grid gap-2 border-t border-[#EEE3D3] bg-white px-3 py-3 text-xs sm:grid-cols-2">
              <div className="rounded-xl bg-[#FFF9F0] px-3 py-2"><p className="text-[9px] font-black uppercase tracking-[0.1em] text-[#8A7863]">{t("reservationsCard.mobile")}</p><a href={`tel:${reservation.phone}`} className="mt-1 block truncate font-semibold text-[#17120D]">{reservation.phone || t("reservationsCard.mobileEmpty")}</a></div>
              <div className="rounded-xl bg-[#FFF9F0] px-3 py-2"><p className="text-[9px] font-black uppercase tracking-[0.1em] text-[#8A7863]">{t("reservationsCard.email")}</p>{reservation.email ? <a href={`mailto:${reservation.email}`} className="mt-1 block truncate font-semibold text-[#17120D]">{reservation.email}</a> : <p className="mt-1 font-semibold text-[#776B5F]">{t("reservationsCard.emailEmpty")}</p>}</div>
              {reservation.offer && <div className="rounded-xl border border-[#E4C993] bg-[#FFF6E5] px-3 py-2 sm:col-span-2"><p className="text-[9px] font-black uppercase tracking-[0.1em] text-[#8A6130]">{t("summary.upcoming.offerApplied")}</p><p className="mt-1 font-semibold text-[#17120D]">{reservation.offer.title} · {marketingBenefitValue(reservation.offer.benefitType, reservation.offer.value == null ? null : Number(reservation.offer.value), reservation.offer.benefitLabel)}</p><p className="mt-0.5 font-mono text-[9px] text-[#806D56]">{reservation.offer.publicCode}</p></div>}
            </div>
          </details>
        )) : <div className="p-6 text-center text-sm text-[#70665B]">{t("summary.upcoming.empty")}</div>}
      </div>
    </div>
  );
}
