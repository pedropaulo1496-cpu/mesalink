...
export default async function PartnerNetworkPage({
  ...
  const defaultDailyCapacity = Math.min(configuredCapacity, restaurant.referralDefaultDailyCapacity > 0 ? restaurant.referralDefaultDailyCapacity : configuredCapacity);
    ...
    <main className="min-h-screen bg-[#F5EFE6] text-[#17120D]">
      <div className="grid min-h-screen lg:grid-cols-[286px_1fr]">
        ...
        <section className="min-w-0 px-4 pb-28 pt-5 sm:px-6 lg:px-8 lg:py-7">
          ...
          {google && <div className={`mt-5 rounded-[22px] border px-5 py-4 text-sm font-semibold ${["connected", "synced", "maps-linked"].includes(google) ? "border-[#A8D3A6] bg-[#EFF9EF] text-[#3F6A4D]" : "border-[#EDC7BB] bg-[#FFF0EA] text-[#A14E36]"}`}>{google === "maps-linked" ? "Restaurante associado ao Google Maps." : google === "connected" ? "Perfil Google associado e mini-perfil atualizado." : google === "synced" ? "Avaliação, reviews e fotografias atualizadas a partir do Google." : "Não foi possível atualizar agora o perfil Google."}</div>}
          ...
          <div className="mt-5 grid gap-5 2xl:grid-cols-[minmax(0,1fr)_380px] 2xl:items-start">
            <div className="space-y-5">
              <section className="rounded-[26px] border border-[#E1D0B8] bg-white p-5 shadow-[0_12px_38px_rgba(88,62,31,0.045)] sm:p-6"><div className="mb-5 flex items-start justify-between gap-4"><div><p className="text-[9px] font-black uppercase tracking-[0.2em] text-[#9B6F3B]">Regra base</p><h2 className="mt-1 text-xl font-semibold tracking-[-0.035em]">Comissão e disponibilidade</h2><p className="mt-1 text-[10px] leading-5 text-[#75695D]">A disponibilidade é cruzada automaticamente com a ocupação real em cada horário.</p></div><span className="hidden h-10 w-10 place-items-center rounded-full bg-[#F4ECE1] text-[#8B6738] sm:grid"><CalendarCheck2 size={17} /></span></div><ReferralBookingSettingsForm restaurantId={id} initialCommissionType={commissionType} initialCommissionAmount={defaultCommissionAmount} initialDefaultDailyCapacity={defaultDailyCapacity} totalCapacity={configuredCapacity} initialAutoAcceptEnabled={restaurant.referralAutoAcceptEnabled} paymentMethodReady={Boolean(restaurant.referralPaymentMethodId)} paymentBlocked={paymentBlocked} paymentBlockReason={restaurant.referralPaymentBlockReason} overdueAmount={overdueAmount} billingDetails={{ legalName: restaurant.billingLegalName || "", taxId: (restaurant.billingTaxId || "").replace(/^PT/i, ""), addressLine1: restaurant.billingAddressLine1 || "", addressLine2: restaurant.billingAddressLine2 || "", postalCode: restaurant.billingPostalCode || "", city: restaurant.billingCity || "" }} initialDailyCapacities={restaurant.referralDailyCapacities.map((item) => ({ date: item.date.toISOString().slice(0, 10), capacity: item.capacity, enabled: item.enabled }))} /></section>
