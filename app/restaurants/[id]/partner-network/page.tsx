...
import { CheckCircle2, CircleDollarSign, Clock3, ShieldCheck, UsersRound } from "lucide-react";
      ...
      <div className="grid min-h-screen lg:grid-cols-[286px_1fr]">
        ...
        <section className="min-w-0 px-4 pb-28 pt-5 sm:px-6 lg:px-8 lg:py-7">
          <header className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div><div className="flex flex-wrap items-center gap-3"><p className="text-xs font-black uppercase tracking-[0.3em] text-[#9B6F3B]">Rede de Parceiros</p><span className={`rounded-full border px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.14em] ${restaurant.referralAutoAcceptEnabled ? "border-[#9CCB9B] bg-[#ECF7EC] text-[#3F6A4D]" : "border-[#D9C7AA] bg-[#FFF8EC] text-[#795D38]"}`}>{restaurant.referralAutoAcceptEnabled ? "Reservas ativas" : "Configuração pendente"}</span></div><h1 className="mt-2 text-3xl font-semibold tracking-[-0.055em] sm:text-4xl">Reservas automáticas de parceiros.</h1><p className="mt-2 max-w-2xl text-sm leading-5 text-[#6B6258]">Tu defines comissão e capacidade. Quando um parceiro escolhe o restaurante, a reserva entra imediatamente.</p></div>
            <div className="flex items-center gap-2 rounded-full border border-[#BAD8B7] bg-[#EFF9EF] px-4 py-2 text-xs font-bold text-[#3F6A4D]"><ShieldCheck size={16} /> Contacto revelado só na reserva</div>
          </header>
          ...
          <section className="mt-4 grid grid-cols-2 gap-2 xl:grid-cols-4">
            <Kpi icon={<UsersRound size={15} />} label="Reservas automáticas" value={String(restaurant.acceptedReferralGroups.filter((group) => group.status === "BOOKED").length)} />
            <Kpi icon={<CircleDollarSign size={15} />} label="Comissão oferecida" value={`${formatMoney(Number(restaurant.referralDefaultCommissionAmount))}${commissionType === "PER_PERSON" ? " / pessoa" : " total"}`} />
            <Kpi icon={<CheckCircle2 size={15} />} label="Reservas concluídas" value={String(completedGroups.length)} />
            <Kpi icon={<Clock3 size={15} />} label="Capacidade habitual" value={`${restaurant.referralDefaultDailyCapacity} lugares / dia`} />
          </section>
          ...
          {restaurant.acceptedReferralGroups.length > 0 && (
            <section className="mt-6 rounded-[34px] border border-[#E1D0B8] bg-[#FFF9F0] p-5 sm:p-8">
              ...
              <h2 className="mt-2 text-3xl font-semibold tracking-[-0.055em]">Reservas de parceiros</h2>
              <p className="mt-2 text-sm leading-6 text-[#6B6258]">Estas reservas entraram automaticamente. Depois da refeição, confirma quantas pessoas vieram ou marca no-show; o pagamento é tratado pelo MesaLink.</p>
          ...
          <section className="mt-5 max-w-3xl rounded-[24px] border border-[#E1D0B8] bg-[#FFF9F0] p-5"><p className="text-[9px] font-black uppercase tracking-[0.2em] text-[#9B6F3B]">Reservas automáticas</p><h2 className="mt-1 text-xl font-semibold tracking-[-0.04em]">Comissão e capacidade</h2><ReferralBookingSettingsForm restaurantId={id} initialCommissionType={commissionType} initialCommissionAmount={Number(restaurant.referralDefaultCommissionAmount)} initialDefaultDailyCapacity={restaurant.referralDefaultDailyCapacity} initialAutoAcceptEnabled={restaurant.referralAutoAcceptEnabled} paymentMethodReady={Boolean(restaurant.referralPaymentMethodId)} initialDailyCapacities={restaurant.referralDailyCapacities.map((item) => ({ date: item.date.toISOString().slice(0, 10), capacity: item.capacity }))} /></section>
            </div>
          </details>
        </section>
      </div>
