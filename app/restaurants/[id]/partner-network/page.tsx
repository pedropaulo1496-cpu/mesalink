            ...
            <div className="mt-6 space-y-3">
              {restaurant.referralOffers.map((offer) => {
                ...
                return <article key={offer.id} className="rounded-[26px] border border-[#E1D0B8] bg-[#FFFDFC] p-4 transition hover:border-[#C9AD83] sm:p-5">
                  <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_220px] lg:items-center">
                    <div className="flex min-w-0 items-start gap-4">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="mt-2 flex flex-wrap gap-x-4 gap-y-2 text-sm text-[#6B6258]"><span className="inline-flex items-center gap-1.5"><CalendarClock size={15} className="text-[#9B6F3B]" />{new Intl.DateTimeFormat("pt-PT", { dateStyle: "medium" }).format(group.desiredDate)}</span><span className="inline-flex items-center gap-1.5"><Clock3 size={15} className="text-[#9B6F3B]" />{new Intl.DateTimeFormat("pt-PT", { timeStyle: "short" }).format(group.desiredDate)}</span><span className="inline-flex items-center gap-1.5"><MapPin size={15} className="text-[#9B6F3B]" />{group.area || group.city || "Zona flexível"}</span></div>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2 sm:ml-auto sm:max-w-sm"><form action={`/api/referral-offers/${offer.id}/decline`} method="POST"><button className="h-11 w-full rounded-full border border-[#D8C6A9] bg-white text-sm font-bold hover:bg-[#FFF7ED]">Rejeitar</button></form><form action={`/api/referral-offers/${offer.id}/accept`} method="POST"><button className="h-11 w-full rounded-full bg-[#17120D] text-sm font-bold text-white hover:bg-[#34271C]">Aceitar</button></form></div>
                </article>;
              })}
              ...
              <div className="mt-6 space-y-3">
                {restaurant.acceptedReferralGroups.map((group) => (
                  <div key={group.id} className="grid gap-4 rounded-[24px] border border-[#E1D0B8] bg-white p-4 sm:grid-cols-[1fr_auto] sm:items-center">
                    <div><div className="flex flex-wrap items-center gap-2"><p className="font-semibold">{group.publicCode}</p><span className="rounded-full border border-[#DCCCAD] bg-[#FFF9ED] px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.12em] text-[#795D38]">{groupStatus(group.status)}</span></div><p className="mt-2 text-sm text-[#6B6258]">{groupPeople(group)} · {new Intl.DateTimeFormat("pt-PT", { dateStyle: "medium", timeStyle: "short" }).format(group.desiredDate)} · comissão {formatMoney(Number(group.payment?.grossCommission || 0))}{group.payment && Number(group.payment.serviceFee) > 0 ? ` + ${formatMoney(Number(group.payment.serviceFee))} serviço` : ""}</p></div>
                    <div>
