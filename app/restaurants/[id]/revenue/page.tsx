...
import { Check, ChevronDown, ChevronRight, CreditCard, Pencil, ShieldCheck, TicketCheck } from "lucide-react";
import BottomNav from "@/components/BottomNav";
import RestaurantSidebar from "@/components/RestaurantSidebar";
import { prisma } from "@/lib/prisma";
import { createExperience, saveRevenueSettings, updateExperienceDetails, updateExperiencePayment, updateExperienceState } from "./actions";
...
export default async function RevenuePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ tab?: string; result?: string; standalone?: string }> }) {
  ...
  const editExperience = updateExperienceDetails.bind(null, id);
  ...
  return <main className="min-h-screen bg-[#F5EFE6] text-[#17120D]"><div className="grid min-h-screen lg:grid-cols-[286px_1fr]">
    ...
    <section className="min-w-0 px-4 pb-28 pt-5 sm:px-6 lg:px-8 lg:pb-8 lg:pt-7">
      ...
      {(tab === "protect" || tab === "experiences") && <><Protect restaurant={restaurant} paymentsReady={paymentsReady} saveSettings={saveSettings} /><Experiences restaurant={restaurant} paymentsReady={paymentsReady} addExperience={addExperience} changeExperience={changeExperience} editExperience={editExperience} changeExperiencePayment={changeExperiencePayment} /></>}
    </section>
...
function Experiences({ restaurant, paymentsReady, addExperience, changeExperience, editExperience, changeExperiencePayment }: { restaurant: RevenueRestaurant; paymentsReady: boolean; addExperience: (formData: FormData) => void; changeExperience: (formData: FormData) => void; editExperience: (formData: FormData) => void; changeExperiencePayment: (formData: FormData) => void }) {
    ...
    <section className="rounded-[30px] border border-[#E1D0B8] bg-white p-5 sm:p-6">
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-2">{restaurant.diningExperiences.length ? restaurant.diningExperiences.map((experience) => {
        ...
        return <article key={experience.id} className="overflow-hidden rounded-[24px] border border-[#E4D5C0] bg-[#FFFDFC]">
          ...
          {experience.addOns.some((addOn) => addOn.active) && <div className="mt-3 flex flex-wrap gap-1.5">{experience.addOns.filter((addOn) => addOn.active).map((addOn) => <span key={addOn.id} className="rounded-full border border-[#E2D0B5] bg-white px-2.5 py-1 text-[10px] font-semibold text-[#6D5C48]">{addOn.name} · {money(Number(addOn.price))}{addOn.perGuest ? "/pessoa" : ""}</span>)}</div>}
          </div>
          <details className="group border-t border-[#E9DDCD] bg-white">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 text-xs font-bold text-[#6F5B44] transition hover:bg-[#FFF9F0] [&::-webkit-details-marker]:hidden"><span className="inline-flex items-center gap-2"><Pencil size={14}/> Editar menu</span><ChevronDown size={15} className="transition group-open:rotate-180"/></summary>
            <form action={editExperience} className="space-y-4 border-t border-[#EEE3D4] bg-[#FFFCF8] p-4">
              <input type="hidden" name="experienceId" value={experience.id}/>
              <div className="grid gap-3 sm:grid-cols-2">
                <EditField label="Nome do menu" wide><input name="title" defaultValue={experience.title} required maxLength={90} className="input-premium"/></EditField>
                <EditField label="Descrição curta" wide><textarea name="summary" defaultValue={experience.summary} required maxLength={320} rows={2} className="input-premium min-h-20 py-3"/></EditField>
                <EditField label="Preço por pessoa"><div className="relative"><input name="pricePerPerson" type="number" min="1" max="5000" step="0.01" defaultValue={String(experience.pricePerPerson)} required className="input-premium pr-10"/><span className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-sm font-bold text-[#8A7761]">€</span></div></EditField>
                <EditField label="Capacidade"><input name="capacity" type="number" min="1" max="1000" defaultValue={experience.capacity} required className="input-premium"/></EditField>
              </div>
              <EditField label="Conteúdo do menu"><textarea name="details" defaultValue={experience.details || ""} maxLength={6000} rows={7} className="input-premium min-h-40 py-3 font-mono text-xs leading-5"/></EditField>
              <div className="grid gap-3 sm:grid-cols-2">
                <fieldset><legend className="mb-1.5 text-[9px] font-black uppercase tracking-[.14em] text-[#806D56]">Disponível ao</legend><div className="grid grid-cols-2 gap-2"><EditCheck name="servicePeriods" value="LUNCH" checked={experience.servicePeriods.includes("LUNCH")} label="Almoço"/><EditCheck name="servicePeriods" value="DINNER" checked={experience.servicePeriods.includes("DINNER")} label="Jantar"/></div></fieldset>
                <EditField label="Disponibilidade"><select name="scheduleType" defaultValue={experience.scheduleType} className="input-premium"><option value="FLEXIBLE">Nas datas disponíveis</option><option value="FIXED">Evento com data fixa</option></select></EditField>
                <EditField label="Data e hora do evento"><input name="startsAt" type="datetime-local" defaultValue={experience.startsAt ? dateTimeLocalValue(experience.startsAt) : ""} className="input-premium"/><span className="mt-1 block text-[10px] leading-4 text-[#8A7C6D]">Só é usada quando selecionas “Evento com data fixa”.</span></EditField>
                <div className="grid grid-cols-2 gap-2"><EditField label="Duração"><select name="durationMinutes" defaultValue={String(experience.durationMinutes)} className="input-premium"><option value="90">1h30</option><option value="120">2h</option><option value="150">2h30</option><option value="180">3h</option></select></EditField><EditField label="Cancelamento"><div className="relative"><input name="cancellationHours" type="number" min="1" max="336" defaultValue={experience.cancellationHours} className="input-premium pr-10"/><span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-[#8A7761]">h</span></div></EditField></div>
              </div>
              <details className="rounded-[17px] border border-[#E4D5C0] bg-white">
                <summary className="cursor-pointer list-none px-3.5 py-3 text-xs font-bold text-[#6F5B44] [&::-webkit-details-marker]:hidden">Editar extras opcionais</summary>
                <div className="space-y-3 border-t border-[#EEE3D4] p-3">
                  {experience.addOns.filter((addOn) => addOn.active).map((addOn) => <div key={addOn.id} className="grid grid-cols-[1fr_90px] gap-2 rounded-[14px] bg-[#FFF9F0] p-2"><input name={`addOnName:${addOn.id}`} defaultValue={addOn.name} aria-label="Nome do extra" className="input-premium"/><input name={`addOnPrice:${addOn.id}`} type="number" min="0" max="1000" step="0.01" defaultValue={String(addOn.price)} aria-label="Preço do extra" className="input-premium"/><label className="flex items-center gap-2 text-[10px] font-semibold text-[#756755]"><input type="checkbox" name={`addOnPerGuest:${addOn.id}`} defaultChecked={addOn.perGuest} className="h-4 w-4 accent-[#17120D]"/> Por pessoa</label><label className="flex items-center justify-end gap-2 text-[10px] font-semibold text-[#A14E36]"><input type="checkbox" name={`removeAddOn:${addOn.id}`} className="h-4 w-4 accent-[#A14E36]"/> Remover</label></div>)}
                  {[1, 2, 3].map((index) => <div key={index} className="grid grid-cols-[1fr_90px] gap-2"><input name={`newAddOnName${index}`} placeholder={index === 1 ? "Novo extra" : "Outro extra"} className="input-premium"/><input name={`newAddOnPrice${index}`} type="number" min="0" max="1000" step="0.01" placeholder="0,00 €" className="input-premium"/><label className="col-span-2 flex items-center gap-2 text-[10px] font-semibold text-[#756755]"><input type="checkbox" name={`newAddOnPerGuest${index}`} className="h-4 w-4 accent-[#17120D]"/> Cobrar por pessoa</label></div>)}
                </div>
              </details>
              <button className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-full bg-[#17120D] px-5 text-xs font-bold text-white"><Check size={15}/> Guardar alterações</button>
            </form>
          </details>
...
function EditField({ label, wide = false, children }: { label: string; wide?: boolean; children: React.ReactNode }) { return <label className={wide ? "block sm:col-span-2" : "block"}><span className="mb-1.5 block text-[9px] font-black uppercase tracking-[.14em] text-[#806D56]">{label}</span>{children}</label>; }
function EditCheck({ name, value, checked, label }: { name: string; value: string; checked: boolean; label: string }) { return <label className="flex h-12 items-center gap-2 rounded-[15px] border border-[#DDC9AB] bg-white px-3 text-xs font-bold"><input type="checkbox" name={name} value={value} defaultChecked={checked} className="h-4 w-4 accent-[#17120D]"/>{label}</label>; }
...
function dateTimeLocalValue(date: Date) { const parts = new Intl.DateTimeFormat("en-CA", { timeZone: "Europe/Lisbon", year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", hourCycle: "h23" }).formatToParts(date); const value = Object.fromEntries(parts.filter((part) => part.type !== "literal").map((part) => [part.type, part.value])); return `${value.year}-${value.month}-${value.day}T${value.hour}:${value.minute}`; }
function resultMessage(result: string) { if (result === "saved") return "Definições guardadas."; if (result === "created") return "Menu publicado e disponível nas reservas."; if (result === "details-updated") return "Conteúdo e disponibilidade do menu atualizados."; if (result === "updated") return "Menu atualizado."; if (result === "payment-updated") return "Entrada e pagamento do menu atualizados."; if (result === "connect-required") return "A opção sem entrada funciona já. Para cobrar entrada ou pré-pagamento, liga primeiro a conta Stripe."; if (result === "invalid") return "Revê os dados do menu e tenta novamente."; if (result === "connected") return "Conta Stripe ligada. Já podes receber entradas opcionais."; return "Alteração concluída."; }
