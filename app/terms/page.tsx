  ...
  return (
    ...
    >
      ...
      <LegalSection title="Catálogo aberto de restaurantes">
        A pesquisa de restaurantes na aplicação Partners utiliza um diretório verificado pelo MesaLink e pode utilizar a Geoapify e dados do OpenStreetMap como complemento de localização. No diretório verificado, as fotografias, horários, descrições e contactos têm origem no site oficial publicamente associado ao estabelecimento, cuja origem e data de verificação são conservadas. Só são publicados restaurantes que mantenham um email público utilizável para o pedido de reserva e uma fotografia real do próprio estabelecimento; não são usadas imagens ilustrativas nem vistas de rua como substituição. As fotografias continuam a pertencer aos respetivos titulares. A origem da classificação é identificada sempre que disponível e não deve ser interpretada como uma avaliação própria do MesaLink. A informação geográfica complementar está sujeita aos <a href="https://www.geoapify.com/terms-and-conditions/" target="_blank" rel="noreferrer" className="font-semibold text-[#9B6F3B] underline">Termos da Geoapify</a> e à <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer" className="font-semibold text-[#9B6F3B] underline">licença OpenStreetMap</a>. Um restaurante apresentado no catálogo só fica reservado depois de confirmação imediata pela rede MesaLink ou de aceitação expressa do pedido pelo próprio restaurante.
      </LegalSection>
