      ...
      {staff.role === "ADMIN" && (
        <section className="mt-4 grid gap-4 xl:grid-cols-[1fr_360px]">
          <div className="rounded-2xl border border-[#DCC9AA] bg-white p-4">
            <p className="text-[9px] font-black uppercase tracking-[0.2em] text-[#9B6F3B]">Liquidação</p><h2 className="mt-1 text-lg font-semibold">Pagar por comercial</h2>
            <div className="mt-3 grid gap-2.5 sm:grid-cols-2">
              {representatives.map((rep) => {
                ...
                return <div key={rep.id} className="rounded-xl border border-[#E2D3BC] bg-[#FFF9F0] p-3"><div className="flex justify-between gap-3"><div><p className="text-[13px] font-bold">{rep.name}</p><p className="mt-0.5 text-[10px] text-[#6B6258]">{rep._count.clients} clientes · {euroAmount(repPaid)} pago</p></div><p className="text-[13px] font-bold text-[#8A6130]">{euroAmount(repPending)}</p></div>{repPending > 0 && <form action={markRepresentativeCommissionsPaid} className="mt-2"><input type="hidden" name="salesRepresentativeId" value={rep.id} /><button className={`${buttonClass} w-full`}>Marcar pago</button></form>}</div>;
              })}
            </div>
          </div>
