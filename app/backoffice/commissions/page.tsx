...
import { commissionPeriod, commissionPeriodLabel, isClosedCommissionPeriod } from "@/lib/sales-commission-statements";
...
export default async function CommissionsPage({ searchParams }: { searchParams: Promise<{ done?: string }> }) {
  ...
  const [commissions, statements, clients] = await Promise.all([
    ...
    prisma.salesCommissionStatement.findMany({
      where,
      include: { salesRepresentative: { select: { name: true } } },
  ...
  const monthlyBalances = [...commissions.reduce((groups, item) => {
    ...
    const current = groups.get(key) || { key, period, salesRepresentativeId: item.salesRepresentativeId, name: item.salesRepresentative.name, gross: 0, commission: 0, paid: 0, pending: 0 };
    ...
    return groups;
  }, new Map<string, { key: string; period: string; salesRepresentativeId: string; name: string; gross: number; commission: number; paid: number; pending: number }>()).values()]
  ...
  return (
    <>
      ...
      <PageHeading eyebrow="Financeiro comercial" title="Comissões" description={staff.role === "ADMIN" ? "Saldos mensais, faturas da equipa e pagamentos. Sem fatura verificada, nenhum valor é liquidado." : "O saldo acumula durante o mês. No mês seguinte, anexa a fatura para o pagamento ser autorizado."} />
      <section className="mt-5 grid gap-2.5 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Saldo acumulado" value={euroAmount(pending)} note="aguarda fecho e/ou fatura" tone="red" />
      ...
      <section className="mt-4 rounded-2xl border border-[#DCC9AA] bg-white p-4">
        <div className="flex items-end justify-between gap-3"><div><p className="text-[9px] font-black uppercase tracking-[0.2em] text-[#9B6F3B]">Fecho mensal</p><h2 className="mt-1 text-lg font-semibold">Saldos e faturas</h2></div><p className="text-[10px] text-[#75695C]">O saldo nunca expira; transita até ficar regularizado.</p></div>
        <div className="mt-3 grid gap-2.5 xl:grid-cols-2">
          {monthlyBalances.map((balance) => {
            ...
            const status = balance.period === currentPeriod ? "ACCUMULATING" : statement?.status || (balance.pending > 0 ? "MISSING" : "PAID");
...
function StatementStatus({ status }: { status: string }) { const labels = { ACCUMULATING: "A acumular", MISSING: "Fatura em falta", PENDING: "Em verificação", VERIFIED: "Pronto a pagar", REJECTED: "Corrigir fatura", PAID: "Pago" } as Record<string, string>; const tone = status === "PAID" || status === "VERIFIED" ? "bg-[#E3F1E2] text-[#35603A]" : status === "MISSING" || status === "REJECTED" ? "bg-[#FFE2D8] text-[#934A35]" : "bg-[#FFF0CA] text-[#80601E]"; return <span className={`rounded-full px-2 py-1 text-[8px] font-black uppercase tracking-wider ${tone}`}>{labels[status] || status}</span>; }
