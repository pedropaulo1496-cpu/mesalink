...
export default async function PartnerPayoutsPage({ searchParams }: { searchParams: Promise<{ done?: string; count?: string }> }) {
  ...
  return <>
    ...
    <section className="mt-4 space-y-2.5">
      {payments.map((payment) => <article key={payment.id} className="rounded-2xl border border-[#DCC9AA] bg-white p-4">
        <div className="grid gap-3 lg:grid-cols-[1.15fr_1fr_0.75fr_0.9fr_auto] lg:items-center">
        ...
        <details className="mt-3 rounded-xl border border-[#E6D8C3] bg-[#FFF9F0]">
          <summary className="cursor-pointer list-none px-3 py-2.5 text-[11px] font-bold text-[#76572F]">Fatura do parceiro · {invoiceStatusLabel(payment.partnerInvoiceStatus)} <span className="float-right text-[10px]">Ver detalhe ↓</span></summary>
          <div className="border-t border-[#E6D8C3] p-3">
