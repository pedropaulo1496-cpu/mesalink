  ...
  return (
    <>
      ...
      <section className="mt-6 rounded-[30px] border border-[#D7B267] bg-[#FFF6E5] p-5 sm:p-6">
        ...
        <div className="mt-5 space-y-3">{activationRequests.map((request) => { const details = activationDetails(request.failureReason); return <article key={request.id} className="rounded-[24px] border border-[#E2D3BC] bg-white p-4"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-bold">{request.restaurant.name}</h3><ActivationStatus status={request.status} /></div><p className="mt-1 text-xs text-[#6B6258]">{request.restaurant.user?.email || "Conta sem email"} · {dateTime(request.sentAt)}</p><div className="mt-3 flex flex-wrap gap-2"><span className="rounded-full bg-[#F3E6D2] px-3 py-1 text-xs font-bold text-[#76552E]">{channelLabel(request.channel)}</span><span className="rounded-full bg-[#EEF5ED] px-3 py-1 text-xs font-bold text-[#3F6A4D]">{details.contactPhone || "Sem contacto"}</span></div>{details.adminNote && <p className="mt-3 text-xs text-[#6B6258]"><strong>Nota:</strong> {details.adminNote}</p>}</div>{staff.role === "ADMIN" && <form action={updateRevenueActivationRequest} className="w-full shrink-0 rounded-2xl bg-[#FFF9F0] p-3 lg:w-[440px]"><input type="hidden" name="requestId" value={request.id} /><div className="grid gap-2 sm:grid-cols-2"><input name="whatsappNumber" defaultValue={details.whatsappNumber || request.restaurant.revenueWhatsappNumber || ""} placeholder="N.º WhatsApp +351…" className={inputClass} /><input name="contentSid" defaultValue={details.contentSid || request.restaurant.revenueWhatsappContentSid || ""} placeholder="Content SID HX…" className={inputClass} /><input name="voiceNumber" defaultValue={details.voiceNumber || request.restaurant.revenueVoiceNumber || ""} placeholder="N.º chamadas +351…" className={inputClass} /><input name="forwardNumber" defaultValue={details.forwardNumber || request.restaurant.revenueVoiceForwardNumber || details.contactPhone} placeholder="Encaminhar para +351…" className={inputClass} /></div><textarea name="adminNote" defaultValue={details.adminNote} rows={2} placeholder="Número atribuído, fornecedor, próxima ação…" className={`${inputClass} mt-2 min-h-20 py-2`} /><div className="mt-2 grid grid-cols-3 gap-2"><button name="status" value="REQUESTED" className="h-10 rounded-xl border border-[#DCC9AA] text-[10px] font-bold">Recebido</button><button name="status" value="PREPARING" className="h-10 rounded-xl bg-[#8A6130] text-[10px] font-bold text-white">Guardar preparação</button><button name="status" value="COMPLETED" disabled={!twilioReady} className="h-10 rounded-xl bg-[#315C36] text-[10px] font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">Ativar canais</button></div></form>}</div></article>; })}{!activationRequests.length && <p className="rounded-2xl border border-dashed border-[#DCC9AA] bg-white p-6 text-center text-sm text-[#6B6258]">Ainda não existem pedidos de ativação.</p>}</div>
      </section>
