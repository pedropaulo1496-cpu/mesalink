    ...
    <>
      ...
      <section className="mt-4 space-y-2.5">
        {requests.map((request) => {
          ...
          return <article key={request.id} className={`rounded-2xl border p-4 ${request.status === "PENDING" ? "border-[#D7B267] bg-white" : "border-[#DCC9AA] bg-white/75"}`}><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="text-[14px] font-bold">{requestLabel(request.type, Number(request.amount || 0), request.durationMonths)}</h2><RequestStatus status={request.status} /></div><p className="mt-1 text-[11px] text-[#6B6258]">{clientName} · {request.salesRepresentative.name} · {dateTime(request.createdAt)}</p><p className="mt-2 max-w-3xl text-[12px] leading-5">{request.reason}</p>{request.adminNote && <p className="mt-2 rounded-xl bg-[#F7F0E5] px-3 py-2 text-[11px] text-[#6B6258]"><strong>Administração:</strong> {request.adminNote}</p>}{request.promotion && <p className="mt-2 text-[11px] font-bold text-[#35603A]">Código enviado: {request.promotion.code} · {request.promotion.status}</p>}</div>{staff.role === "ADMIN" && request.status === "PENDING" && <form action={decideCommercialRequest} className="w-full shrink-0 rounded-xl border border-[#E2D3BC] bg-[#FFF9F0] p-2.5 lg:w-72"><input type="hidden" name="requestId" value={request.id} /><textarea name="adminNote" rows={2} placeholder="Nota opcional" className={`${inputClass} min-h-16 py-2`} /><div className="mt-2 grid grid-cols-2 gap-2"><button name="decision" value="REJECT" className="h-9 rounded-lg border border-[#E7B7A8] text-[11px] font-bold text-[#9C412B]">Recusar</button><button name="decision" value="APPROVE" className="h-9 rounded-lg bg-[#315C36] text-[11px] font-bold text-white">Aprovar</button></div></form>}</div></article>;
        })}
