...
async function renderSupportInbox(staff: StaffIdentity, requestedConversationId?: string, done?: string) {
  ...
  return <>
    ...
    <div className="mt-4"><ChatTabs mode="clients" /></div>
    {!selected ? <section className="mt-5 rounded-2xl border border-[#DCC9AA] bg-white p-8 text-center"><MessageCircleMore className="mx-auto text-[#A97936]" size={34} /><h2 className="mt-3 text-2xl font-semibold">Ainda não há mensagens de clientes</h2><p className="mt-2 text-sm text-[#6B6258]">As novas conversas aparecem aqui automaticamente.</p></section> :
      <section className="grid min-h-[620px] overflow-hidden rounded-[24px] border border-[#DCC9AA] bg-white shadow-[0_14px_40px_rgba(72,48,21,0.06)] lg:h-[calc(100vh-285px)] lg:min-h-[620px] lg:max-h-[780px] lg:grid-cols-[300px_minmax(0,1fr)]">
        <aside className="border-b border-[#E2D3BC] bg-[#FFF9F0] p-3 lg:min-h-0 lg:border-b-0 lg:border-r"><p className="px-2 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-[#9B6F3B]">Conversas com clientes</p><div className="flex gap-2 overflow-x-auto pb-2 lg:block lg:h-[calc(100%-38px)] lg:space-y-1 lg:overflow-y-auto lg:pb-0">{conversations.map((conversation) => {
