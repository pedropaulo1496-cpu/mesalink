...
export function hasValidHeroImage(restaurant: PublicRestaurant) {
  return Boolean(restaurant.websiteHeroImage?.startsWith("http"));
}

export function hasValidLogo(restaurant: PublicRestaurant) {
  return Boolean(restaurant.websiteLogoImage?.startsWith("http"));
}
