...
export async function POST(request: Request, { params }: { params: Promise<{ benefitId: string }> }) {
  ...
  for (const customer of customers) {
    ...
    try {
      ...
      reserved = true;
      card = await prisma.marketingPromoCard.create({ data: { publicCode: createBenefitCardCode(), restaurantId: benefit.restaurantId, customerId: customer.id, campaignId: benefit.id, title: benefit.title, description: benefit.description, benefitType, benefitLabel: benefit.benefitLabel, value: benefitType === "GIFT" ? null : value, minSpend: benefit.minSpend, terms: benefit.terms, template: benefit.template, expiresAt: benefit.validUntil } });
      ...
      const delivery = await resend.emails.send({
        ...
        html: cardEmail({ restaurantName: benefit.restaurant.name, customerName: customer.name, title: benefit.title, description: benefit.description, publicCode: card.publicCode, cardUrl, benefitType, benefitLabel: benefit.benefitLabel, value, expiry: benefit.validUntil, openUrl, theme }),
      });
