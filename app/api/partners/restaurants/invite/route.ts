...
export async function POST(request: Request) {
  ...
  const introduction = networkInvite
    ...
    : `${escapeHtml(partnerName)} convidou o seu restaurante a aderir ao MesaLink para lhe enviar clientes. Cada reserva representa mais mesas ocupadas e mais faturação para o restaurante.`;
  const delivery = await new Resend(process.env.RESEND_API_KEY).emails.send({
    ...
    subject,
    text: `Olá,\n\n${networkInvite ? `${partnerName} gostaria de recomendar ${restaurantName} aos seus clientes e convidou-o a conhecer a rede MesaLink.` : `${partnerName} convidou ${restaurantName} a aderir ao MesaLink para lhe enviar clientes. Cada reserva representa mais mesas ocupadas e mais faturação para o restaurante.`}\n\n${networkInvite ? "Na MesaLink pode receber e gerir reservas, definir disponibilidade e comissão e acompanhar os pedidos." : `A comissão é definida por pessoa e negociada diretamente entre ${restaurantName} e ${partnerName}. O restaurante mantém sempre o controlo da disponibilidade, dos horários e das reservas que recebe.`}\n\nCriar conta: ${registerUrl}\n\nEste convite foi feito por ${partnerName} (${partner.email}). Pode responder diretamente a este email para falar com o parceiro.\n\nEquipa MesaLink\ninfo@mesalink.pt`,
    html: `<div style="margin:0;background:#f5efe6;padding:28px 14px;font-family:Arial,sans-serif;color:#17120d"><div style="max-width:620px;margin:auto;overflow:hidden;border:1px solid #dfcfb8;border-radius:22px;background:white"><div style="background:#17120d;padding:24px 28px;color:white"><strong style="font:700 26px Georgia,serif"><span style="color:#d7b267">Mesa</span>Link</strong></div><div style="padding:30px 28px"><p style="font-size:11px;font-weight:700;letter-spacing:1.6px;text-transform:uppercase;color:#9b6f3b">${networkInvite ? "Convite para a rede MesaLink" : "Novos clientes e mais faturação"}</p><h1 style="margin:10px 0 16px;font:700 29px/1.16 Georgia,serif">${heading}</h1><p style="font-size:15px;line-height:1.65;color:#685d52">${introduction}</p><div style="margin:20px 0;border:1px solid #e4d3b9;border-radius:16px;background:#fff9ef;padding:17px;font-size:14px;line-height:1.7">${networkInvite ? "<strong>Na MesaLink pode:</strong><br>Receber e gerir reservas<br>Definir disponibilidade e comissão<br>Acompanhar confirmações e alterações" : `<strong>Como funciona:</strong><br>${escapeHtml(partnerName)} envia-lhe clientes através do MesaLink.<br>A comissão é definida por pessoa e negociada diretamente entre ambos.<br>O restaurante controla a disponibilidade, os horários e as reservas.`}</div><a href="${registerUrl}" style="display:inline-block;border-radius:999px;background:#17120d;padding:15px 24px;color:white;text-decoration:none;font-weight:700">${networkInvite ? "Conhecer e aderir à MesaLink" : "Começar a receber clientes"}</a><p style="margin-top:22px;font-size:12px;line-height:1.55;color:#918577">Convite enviado por ${escapeHtml(partnerName)}. As respostas a este email seguem para ${escapeHtml(partner.email)}.</p></div></div></div>`,
