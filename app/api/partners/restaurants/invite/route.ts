...
export async function POST(request: Request) {
  ...
  const kind = body?.kind === "JOIN_NETWORK" ? "JOIN_NETWORK" : "INSTANT_BOOKING";
  ...
  const networkInvite = kind === "JOIN_NETWORK";
  ...
  const heading = networkInvite
    ? `${escapeHtml(partnerName)} quer recomendar ${escapeHtml(place.name)} através da MesaLink.`
    : `${escapeHtml(place.name)} pode começar a aceitar reservas imediatas.`;
  ...
  }, { idempotencyKey: `partner-invite-${kind.toLowerCase()}-${partner.id}-${createHash("sha256").update(placeId).digest("hex").slice(0, 24)}` });
