...
const REGISTER_URL = "https://www.mesalink.pt/register?utm_source=partner_invite&utm_medium=email&utm_campaign=restaurant_network";
...
export async function POST(request: Request) {
  ...
  const destinationUrl = existingRestaurant?.userId
    ? `https://www.mesalink.pt/login?callbackUrl=${encodeURIComponent(`/restaurants/${existingRestaurant.id}/partner-network`)}`
    : REGISTER_URL;
  ...
  }, { idempotencyKey: `restaurant-referral-${partner.id}-${createHash("sha256").update(email).digest("hex").slice(0, 24)}` });
