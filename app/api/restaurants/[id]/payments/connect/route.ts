...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const link = await stripe.accountLinks.create({
      account: accountId,
      refresh_url: `${origin}/restaurants/${id}/experiences?result=connect-required#protecao-no-show`,
    ...
    return NextResponse.redirect(new URL(`/restaurants/${id}/experiences?result=connect-required#protecao-no-show`, request.url), 303);
  }
}
