...
async function ownedRestaurant(id: string, email: string) {
  return prisma.restaurant.findFirst({
    where: { id, user: { email } },
    select: { id: true, reservationMode: true, totalCapacity: true, tables: { select: { capacity: true } } },
  });
}
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const restaurant = await ownedRestaurant(id, session.user.email);
  ...
  if (enabled && (capacity < 1 || capacity > configuredCapacity)) {
