  ...
  try {
    ...
    for (const customer of customers) {
      ...
      try {
        ...
        const offer = configuredOffer || (restaurant.birthdayOffer ? { title: "O seu presente de aniversário", description: restaurant.birthdayOffer, benefitType: "GIFT", value: null, minSpend: null, validDays: 30, terms: null, template: "GOLD" as MarketingCardTheme } : null);
        promoCard = offer ? await prisma.marketingPromoCard.create({
          data: {
            ...
            benefitType: offer.benefitType, value: offer.value, minSpend: offer.minSpend, terms: offer.terms,
...
function parseBirthdayOffer(value: unknown): { offer: BirthdayOffer | null; error?: string } {
  ...
  return { offer: { title, description, benefitType: benefitType as BirthdayOffer["benefitType"], value: benefitType === "GIFT" ? null : numericValue, minSpend: minSpend || null, validDays, terms, template } };
}
