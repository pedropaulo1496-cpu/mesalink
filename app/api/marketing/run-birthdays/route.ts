...
export async function POST(request: Request) {
  ...
  const parsedOffer = parseBirthdayOffer(body?.offer);
  if (parsedOffer.error || !parsedOffer.offer) return NextResponse.json({ success: false, error: parsedOffer.error || "Escolhe a recompensa de aniversário." }, { status: 400 });
  ...
  return runBirthdays(restaurantId, parsedOffer.offer);
}
...
export async function GET(request: Request) {
  ...
  return runBirthdays(null, null);
}

async function runBirthdays(restaurantId: string | null, configuredOffer: BirthdayOffer | null) {
  try {
    ...
    for (const customer of customers) {
      ...
      try {
        ...
        const offer = configuredOffer || (restaurant.birthdayOffer ? { title: "O seu presente de aniversário", description: restaurant.birthdayOffer, benefitType: "GIFT", benefitLabel: restaurant.birthdayOffer, value: null, minSpend: null, validDays: 30, terms: null, template: "GOLD" as MarketingCardTheme } : null);
        promoCard = offer ? await prisma.marketingPromoCard.create({
          data: {
            ...
            campaignId: executionId, title: offer.title, description: offer.description,
            benefitType: offer.benefitType, benefitLabel: offer.benefitLabel, value: offer.value, minSpend: offer.minSpend, terms: offer.terms,
            template: offer.template, expiresAt: new Date(Date.now() + offer.validDays * 24 * 60 * 60 * 1000),
          },
        }) : null;
        ...
        const delivery = await resend.emails.send({
          ...
          html: birthdayEmailHtml({ restaurantName: restaurant.name, customerName: customer.name, offer, clickUrl, openUrl, cardUrl, publicCode: promoCard?.publicCode || null }),
        });
...
function parseBirthdayOffer(value: unknown): { offer: BirthdayOffer | null; error?: string } {
  ...
  const template = String(input.template || "GOLD").toUpperCase() as MarketingCardTheme;
  ...
  if (invalid) return { offer: null, error: "Revê a recompensa, a validade e os dados do cartão." };
  return { offer: { title, description, benefitType: benefitType as BirthdayOffer["benefitType"], benefitLabel: benefitType === "GIFT" ? benefitLabel : null, value: benefitType === "GIFT" ? null : numericValue, minSpend: minSpend || null, validDays, terms, template } };
}
